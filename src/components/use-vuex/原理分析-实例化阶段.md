...
2. 接着就是构建 Vuex 的 Store 对象，在 Store 实例化过程中，注册和维护了模块集合对象，处理了 dispatch 和 commit 指向，递归安装了各个 module, 之后在 Store 对象内部构建了一个 vue 实例，这个

Vue 实例的 data 关联的是 state 对象，这样当我们在后续各个组件内部使用 this.$store.state 时，响应式数据通过 getter 完成了对相关组件的依赖收集，后续 state 的变更，就能触发不同组件的更新了。
